...
for iDataSet in range(nDataSet):
    ...
    mapping_network_optim = torch.optim.Adam(mapping_network.parameters(), lr=args.learning_rate)
    semantic_feature_encoder_optim = torch.optim.Adam(semantic_feature_encoder.parameters(), lr=args.learning_rate)
    ...
    for episode in range(5001):
        ...
        mix_querys_2_sou, query_label_21_sou, query_label_22_sou, lam_21_sou, lam_22_sou = cutmix2(querys_sou,
                                                                                                   ...
                                                                                                   query_att_tar,
                                                                                                   alpha=1.0)
        mix_query_features_1_sou, _ = feature_encoder(mix_querys_1_sou.cuda())
        mix_query_features_2_sou, _ = feature_encoder(mix_querys_2_sou.cuda())
        ...
        mix_querys_2_tar, query_label_21_tar, query_label_22_tar, lam_21_tar, lam_22_tar = cutmix2(querys_tar,
                                                                                                   ...
                                                                                                   query_att_sou,
                                                                                                   alpha=1.0)
        mix_query_features_1_tar, _ = feature_encoder(mix_querys_1_tar.cuda())
        mix_query_features_2_tar, _ = feature_encoder(mix_querys_2_tar.cuda())
        ...
        f_loss_sou = torch.mean(f_loss_sou)
        f_loss_1_sou = torch.mean(f_loss_1_sou)
        f_loss_2_sou = torch.mean(f_loss_2_sou)
        ...
        f_loss_tar = torch.mean(f_loss_tar)
        f_loss_1_tar = torch.mean(f_loss_1_tar)
        f_loss_2_tar = torch.mean(f_loss_2_tar)
        ...
        f_loss_joint_sou = torch.mean(
            F.cross_entropy(joint_logits_sou, query_labels_sou.long().cuda(), reduction="none"))
        f_loss_joint_tar = torch.mean(
            F.cross_entropy(joint_logits_tar, query_labels_tar.long().cuda(), reduction="none"))
