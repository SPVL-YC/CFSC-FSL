...
labels_tar = [
    "Asphalt: A black cementitious material used for paving roads, parking lots, and airport runways. Low reflectance with smooth spectral variation and homogeneous, elongated spatial patterns.",
    "Meadows: Open areas covered with grass and often wildflowers, typically found in rural or natural settings. Vegetation-like spectra with high NIR reflectance and relatively smooth, homogeneous spatial patterns.",
    "Gravel: A surface covered with small stones or pebbles, commonly used for driveways and walkways. Moderate reflectance with coarse, heterogeneous spatial textures.",
    "Trees: Areas dominated by large plants with woody stems and leaves, forming a forest or wooded area. Low red and high NIR reflectance with dense, textured canopy patterns.",
    "Sheets: Flat, thin layers, possibly referring to metal sheets, plastic sheets, or other similar materials. Relatively bright spectral responses with compact and regular geometric spatial patterns.",
    "Bare soil: Soil that is exposed without vegetation cover, often seen in construction sites or eroded areas. Moderate reflectance with smooth spectral variation and irregular, continuous spatial regions.",
    "Bitumen: A sticky, black, and highly viscous liquid or semi-solid form of petroleum used for paving and roofing. Very low reflectance with smooth spectra and compact, homogeneous spatial patterns.",
    "Bricks: Solid blocks of clay or concrete, usually used as building materials for walls and structures. Moderate-to-high visible reflectance with regular, textured geometric spatial patterns.",
    "Shadow: Dark areas produced by the obstruction of light, often seen under objects or structures. Very low reflectance across bands with irregular spatial shapes following surrounding objects."
]
...
def _init_():
    ...
    if not os.path.exists('./checkpoints_vtaf/UP'):
        os.makedirs('./checkpoints_vtaf/UP')
                                                  
    if not os.path.exists('./classificationMap_vtaf/UP'):
        os.makedirs('./classificationMap_vtaf/UP')
...
test_data = '/data/yuchao/data/yuchao/Datasets/PU/paviaU.mat'            
test_label = '/data/yuchao/data/yuchao/Datasets/PU/paviaU_gt.mat'           
            ...
            with torch.no_grad():
                ...
                if test_accuracy > last_accuracy:
                    ...
                    torch.save(mapping_network.state_dict(),
                               str("./checkpoints_vtaf/UP/DFSL_mapping_network_" + "IP_" + str(iDataSet) + "iter_" + str(
                                   TEST_LSAMPLE_NUM_PER_CLASS) + "shot.pkl"))
                    torch.save(feature_encoder.state_dict(),
                               str("./checkpoints_vtaf/UP/DFSL_feature_encoder_" + "IP_" + str(iDataSet) + "iter_" + str(
                                   TEST_LSAMPLE_NUM_PER_CLASS) + "shot.pkl"))
                    ...
                    torch.save({
                        ...
                        "cma_temperature": 0.1,
                    }, f"./checkpoints_vtaf/UP/VTAF_UP_{iDataSet}iter_{TEST_LSAMPLE_NUM_PER_CLASS}shot.pkl")
...
#
# utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24, "./UP_{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))
