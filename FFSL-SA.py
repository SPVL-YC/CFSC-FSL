...
import time
import utils_vtaf as utils
import sys
from torchvision import transforms
from vit_vtaf import Transformer, CrossModalAttention
import vit_vtaf as vit
...
parser.add_argument("-d", "--tar_input_dim", type=int, default=204, help="Dimension of the input data in the target domain")
...
parser.add_argument("-m", "--test_class_num", type=int, default=16, help="Number of classes in the target domain")
...
labels_sou = [
    "water: Large areas of water bodies, such as lakes, rivers, or oceans. Low reflectance, especially in the NIR, with smooth and homogeneous spatial patterns."
    "bare soil school: Bare soil areas found within school premises, potentially from construction or activity fields. Moderate reflectance with compact and relatively regular spatial regions.",
    "bare soil park: Exposed ground sections in parks, commonly seen in walkways or recreational zones. Moderate soil-like reflectance with locally continuous and irregular spatial patterns.",
    "bare soil farmland: Uncovered earth patches in agricultural fields, likely due to plowing or fallow land. Moderate-to-high reflectance with large and regular field-like spatial patterns.",
    "natural plants: Vegetation that grows naturally, such as forests, grasslands, or wild flora. Low red and high NIR reflectance with heterogeneous spatial patterns.",
    "weeds in farmland: Weeds present in crop fields, often competing with cultivated plants for resources. Vegetation-like spectra with high NIR reflectance and scattered spatial distributions.",
    "forest: Dense woodland areas, comprising a variety of trees and understory vegetation. Low red and high NIR reflectance with dense and textured canopy patterns.",
    "grass: Grassy areas, which could be natural grasslands or artificial turf. Vegetation-like spectra with high NIR reflectance and relatively uniform spatial patterns.",
    "rice field grown: Rice fields in the growth stage, where the rice plants have developed to a certain height. Strong vegetation spectra with high NIR reflectance and regular field structures.",
    "rice field first stage: The initial stage of rice fields, where rice plants are just beginning to grow. Mixed vegetation-soil/water spectra with regular field boundaries.",
    "row crops: Crops planted in rows, such as corn, soybeans, or other cultivated plants. Vegetation-dominated spectra with high NIR reflectance and repetitive row patterns.",
    "plastic house: Greenhouses or hoop houses covered with plastic sheeting for temperature control. Distinctive bright spectral responses with regular geometric spatial patterns.",
    "manmade non dark: Artificial constructions with a light hue, like white or light-colored buildings. High visible reflectance with sharp and regular geometric patterns.",
    "manmade dark: Man-made structures that are dark in color, such as black or dark grey buildings. Low overall reflectance with compact and regular spatial structures.",
    "manmade blue: Constructed edifices coated in azure, potentially indicating specific usage or ownership. Stronger blue-band reflectance with compact and regular roof-like patterns.",
    "manmade red: Synthetic constructions coated with red paint, which might signify particular functions or affiliations. Stronger red-band reflectance with compact and regular building-like patterns.",
    "manmade grass: Human-made turf, often used for sports fields or decorative purposes. Uniform spectral responses with highly regular and homogeneous spatial layouts.",
    "asphalt: Paved surfaces made of asphalt, commonly found in roads or parking lots. Low reflectance with smooth spectral variation and elongated or homogeneous spatial patterns.",
    "paved ground: Ground covered with paving materials, such as concrete or brick for sidewalks. Moderate reflectance with smooth spectral variation and regular spatial patterns."
]

labels_tar = [
    "Brocoli green weeds 1: A kind of green Herbaceous Plant found in broccoli farms. that may affect crop growth. Vegetation-like spectra with high NIR reflectance and relatively compact field patterns.",
    "Brocoli green weeds 2: Another type of green weed in broccoli fields, different from the first one. Similar vegetation spectra with high NIR reflectance and slightly different local texture patterns.",
    "Fallow: Land left uncultivated to recover its fertility or to control pests and weeds. Soil-dominated spectra with moderate reflectance and broad, homogeneous field regions.",
    "Fallow rough plow: Roughly plowed fallow land, with a relatively rough soil surface. Soil-like spectra with strong spatial texture and irregular rough patterns.",
    "Fallow smooth: Smoothly plowed fallow land, with a more even soil surface. Soil-like spectra with relatively smooth and homogeneous spatial patterns.",
    "Stubble: The remaining stalks of crops after harvesting, such as wheat or corn residues. Dry-vegetation and soil-mixed spectra with fragmented, textured field patterns.",
    "Celery: Fields where celery is grown, designated areas for celery cultivation. Vegetation-dominated spectra with high NIR reflectance and regular crop-field structures.",
    "Grapes untrained: Vineyards where grapevines are growing without training or support. Strong vegetation spectra with heterogeneous and irregular vine patterns.",
    "Soil vinyard develop: Soil in a developing vineyard, likely undergoing soil improvement processes. Soil-dominated spectra with sparse vegetation influence and irregular field patterns.",
    "Corn senesced green weeds: Aging corn plants in a field coexisting with green weeds. Mixed senescent-crop and green-vegetation spectra with heterogeneous spatial textures.",
    "Lettuce romaine 4wk: At the 4-week mark, Romaine lettuce is just beginning to sprout and is in its nascent phase. Weak vegetation responses with visible soil background and sparse, regular planting patterns.",
    "Lettuce romaine 5wk: When Romaine lettuce reaches 5 weeks, it is halfway through its growth cycle. Vegetation responses with moderately dense and regular crop patterns.",
    "Lettuce romaine 6wk: By the 6-week point, Romaine lettuce is nearing its full size and ripeness. Strong vegetation spectra with dense and regular field structures.",
    "Lettuce romaine 7wk: At 7 weeks, Romaine lettuce has reached complete maturity. Vegetation responses with highly dense and homogeneous crop patterns.",
    "Vinyard untrained: Untrained grapevines in vineyards, growing spontaneously and freely. Vegetation-dominated spectra with irregular, heterogeneous vineyard structures.",
    "Vinyard vertical trellis: Vineyards with a vertical trellis system where grapevines are trained to grow along supports. Vegetation spectra with distinctive regular and repetitive row-like structures."
]
...
labels_tar_all = [
    ...
    Lettuce romaine 5wk: When Romaine lettuce reaches 5 weeks, it is halfway through its growth cycle. Vegetation responses with moderately dense and regular crop patterns,\
    Lettuce romaine 6wk: By the 6-week point, Romaine lettuce is nearing its full size and ripeness. Strong vegetation spectra with dense and regular field structures,\
    Lettuce romaine 7wk: At 7 weeks, Romaine lettuce has reached complete maturity. Strong vegetation responses with highly dense and homogeneous crop patterns,\
    Vinyard untrained: Untrained grapevines in vineyards, growing spontaneously and freely. Vegetation-dominated spectra with irregular, heterogeneous vineyard structures,\
    Vinyard vertical trellis: Vineyards with a vertical trellis system where grapevines are trained to grow along supports. Vegetation spectra with distinctive regular and repetitive row-like structures."
]
...
labels_sou_all = [" ".join(labels_sou)]
labels_tar_all = [" ".join(labels_tar)]
...
semantic_mapping_sou = outputs_sou.last_hidden_state.detach().cpu()
semantic_mask_sou = encoded_inputs_sou["attention_mask"].bool().cpu()
...
semantic_mapping_tar = outputs_tar.last_hidden_state.detach().cpu()
semantic_mask_tar = encoded_inputs_tar["attention_mask"].bool().cpu()
...
semantic_mapping_sou_all = outputs_sou_all.last_hidden_state.detach().cpu()
semantic_mask_sou_all = encoded_inputs_sou_all["attention_mask"].bool().cpu()
...
semantic_mapping_tar_all = outputs_tar_all.last_hidden_state.detach().cpu()
semantic_mask_tar_all = encoded_inputs_tar_all["attention_mask"].bool().cpu()


def _init_():
    if not os.path.exists('./checkpoints_vtaf/SA'):
        os.makedirs('./checkpoints_vtaf/SA')
    if not os.path.exists('./classificationMap_vtaf/SA'):
        os.makedirs('./classificationMap_vtaf/SA')
...
test_data = '/data/yuchao/data/yuchao/Datasets/salinas/salinas_corrected.mat'
test_label = '/data/yuchao/data/yuchao/Datasets/salinas/salinas_gt.mat'
...
class Network(nn.Module):
    ...
    def forward(self, x, return_tokens=False):
        ...
        global_feature = x[:, -1, :]
        if return_tokens:
            visual_tokens = torch.cat((x[:, -1:, :], x[:, :-1, :]), dim=1)
            return global_feature, att, visual_tokens
        return global_feature, att
...
image_text_contrastive_loss = utils.ImageTextContrastiveLoss(temperature=0.1).cuda()
...
seeds = [1180, 1048, 1147, 1283, 1319, 1395, 1399, 1353, 1351, 1405]
...
for iDataSet in range(nDataSet):
    ...
    semantic_feature_encoder = vit.WordEmbTransformers(feature_dim=N_DIMENSION, dropout=0.5)
    ...
    attn_layer = CrossModalAttention(feat_dim=N_DIMENSION, attn_dim=N_DIMENSION, dropout=0.1).cuda()
    attn_layer.train()
    attn_layer_optim = torch.optim.Adam(attn_layer.parameters(), lr=args.learning_rate)
    print("Training...")
    ...
    for episode in range(5001):
        ...
        semantic_features_sou, semantic_features_sou_all, text_mask_sou, text_mask_sou_all = vit.map_episode_text(
            semantic_feature_encoder, semantic_mapping_sou, semantic_mask_sou,
            semantic_mapping_sou_all, semantic_mask_sou_all, query_real_labels_sou)
        ...
        semantic_features_tar, semantic_features_tar_all, text_mask_tar, text_mask_tar_all = vit.map_episode_text(
            semantic_feature_encoder, semantic_mapping_tar, semantic_mask_tar,
            semantic_mapping_tar_all, semantic_mask_tar_all, query_real_labels_tar)
        ...
        mix_query_features_1_sou, mix_query_outputs_1_sou, visual_tokens_sou = feature_encoder(
            mix_querys_1_sou.cuda(), return_tokens=True)
        ...
        mix_query_features_1_tar, mix_query_outputs_1_tar, visual_tokens_tar = feature_encoder(
            mix_querys_1_tar.cuda(), return_tokens=True)
        ...
        text_tokens_sou, text_global_sou, fusion_mask_sou = vit.compose_text_tokens(
            semantic_features_sou, semantic_features_sou_all,
            lam_11_sou, lam_12_sou, text_mask_sou, text_mask_sou_all)
        text_align_loss_sou = image_text_contrastive_loss(
            mix_query_features_1_sou, text_global_sou)
        joint_feature_sou = attn_layer(
            visual_tokens_sou, text_tokens_sou, fusion_mask_sou)

        text_tokens_tar, text_global_tar, fusion_mask_tar = vit.compose_text_tokens(
            semantic_features_tar, semantic_features_tar_all,
            lam_11_tar, lam_12_tar, text_mask_tar, text_mask_tar_all)
        text_align_loss_tar = image_text_contrastive_loss(
            mix_query_features_1_tar, text_global_tar)
        joint_feature_tar = attn_layer(
            visual_tokens_tar, text_tokens_tar, fusion_mask_tar)
        ...
        loss = f_loss + 2 * (f_loss_1 +  f_loss_2 ) + text_align_loss + f_loss_joint


        feature_encoder.zero_grad()
        mapping_network.zero_grad()
        semantic_feature_encoder.zero_grad()
        attn_layer_optim.zero_grad(set_to_none=True)
        loss.backward()
        feature_encoder_optim.step()
        mapping_network_optim.step()
        semantic_feature_encoder_optim.step()
        attn_layer_optim.step()
            ...
            with torch.no_grad():
                ...
                if test_accuracy > last_accuracy:
                    torch.save(mapping_network.state_dict(),
                               str("./checkpoints_vtaf/SA/DFSL_mapping_network_" + "IP_" + str(iDataSet) + "iter_" + str(
                                   TEST_LSAMPLE_NUM_PER_CLASS) + "shot.pkl"))
                    torch.save(feature_encoder.state_dict(),
                               str("./checkpoints_vtaf/SA/DFSL_feature_encoder_" + "IP_" + str(iDataSet) + "iter_" + str(
                                   TEST_LSAMPLE_NUM_PER_CLASS) + "shot.pkl"))
                    torch.save({
                        "episode": episode,
                        "mapping_network": mapping_network.state_dict(),
                        "feature_encoder": feature_encoder.state_dict(),
                        "semantic_feature_encoder": semantic_feature_encoder.state_dict(),
                        "vtaf": attn_layer.state_dict(),
                        "feature_optimizer": feature_encoder_optim.state_dict(),
                        "mapping_optimizer": mapping_network_optim.state_dict(),
                        "semantic_optimizer": semantic_feature_encoder_optim.state_dict(),
                        "vtaf_optimizer": attn_layer_optim.state_dict(),
                        "cma_temperature": 0.1,
                    }, f"./checkpoints_vtaf/SA/VTAF_SA_{iDataSet}iter_{TEST_LSAMPLE_NUM_PER_CLASS}shot.pkl")
...
# utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24, "./SA_{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))