...
def radiation_noise(data):
    argument = (np.random.randint(1, 10), np.random.randint(1, 10))  
    da_1 = transforms.CenterCrop(argument)  
    da_2 = transforms.Resize((9, 9))  
    data = data.transpose(2, 0, 1) 
    x = da_1(torch.from_numpy(data))  
    x = da_2(x) 
    x = x.numpy() 
    x = x.transpose(1, 2, 0)  
    return x  
...
class ImageTextContrastiveLoss(nn.Module):
    """CMA, Eq. (19): cosine NT-Xent over 2B paired global features.

    Batch size and device come from the inputs; the optional legacy arguments
    remain accepted so callers need not construct a fixed-size CUDA mask.
    """

    def __init__(self, batch_size=None, device=None, temperature=0.1):
        super().__init__()
        if temperature <= 0:
    ...
    def forward(self, emb_i, emb_j):
        if emb_i.ndim != 2 or emb_i.shape != emb_j.shape or emb_i.shape[0] == 0:
            raise ValueError("CMA expects matched, nonempty [B, D] tensors")
        batch_size = emb_i.shape[0]
        representations = F.normalize(torch.cat((emb_i, emb_j), dim=0), dim=-1)
        logits = representations @ representations.transpose(0, 1) / self.temperature
        diagonal = torch.eye(2 * batch_size, dtype=torch.bool, device=logits.device)
        logits = logits.masked_fill(diagonal, float("-inf"))
        positives = (torch.arange(2 * batch_size, device=logits.device) + batch_size) % (2 * batch_size)
